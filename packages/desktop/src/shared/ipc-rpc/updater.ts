...
export const UpdaterStateSchema = Schema.Union([
  ...
  // Os dois seguintes nascem no updater do `web` (troca de binário + reinício do
  // servidor). O electron-updater nunca os emite, mas o schema descreve o `UpdaterState`
  // que o app inteiro consome — sem eles o contrato IPC deixa de aceitar o tipo.
  Schema.Struct({ status: Schema.Literal("restarting"), version: Schema.String }),
  Schema.Struct({
    status: Schema.Literal("manual"),
