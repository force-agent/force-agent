...
it.live("activates credentials through the HttpApi", () =>
  Effect.gen(function* () {
    const handler = yield* ServerFetch.make(options, { grant: InProcess.grant() })
    const response = yield* Effect.promise(() =>
...
// An in-process grant replaces the password: the routes are still authenticated,
// the owner stamps the minted credential, and no unauthenticated graph exists.
it.live("authenticates an in-process grant and refuses an unconfigured CORS origin", () =>
  Effect.gen(function* () {
    const handler = yield* ServerFetch.make(options, { grant: InProcess.grant() })
    ...
    // Localhost is only an allowed origin behind the development flag, and the
    // origins upstream granted unconditionally (opencode.ai, oc://renderer,
    // tauri) are gone.
    for (const origin of ["http://localhost:3000", "https://opencode.ai", "oc://renderer", "tauri://localhost"]) {
    ...
    ).pipe(Effect.ensuring(Effect.sync(() => delete process.env["POWER_DEV_CORS"])))
    expect(development.headers.get("access-control-allow-origin")).toBe("http://localhost:3000")
  }),
)
