...
import { hasBrowserStreamTicketURL } from "@opencode-ai/protocol/groups/browser"
...
export const start = Effect.fn("ServerProcess.start")(function* <E, R>(
  ...
  transform?: Transform,
  // Runtime-profile replacements handed to createRoutes (e.g. the CLI's self-update applier).
  overrides: LayerNode.Replacements = [],
) {
  const password = options.password
  // power-agent overlay: `usable` rather than truthiness — a secret that is
  // empty or only whitespace is not a credential. An embedder calling start()
  // directly bypasses the CLI check, so the refusal has to live here too.
  if (!ServerAuth.usable(password)) return yield* Effect.fail(new Error("Missing server password"))
...
function dispatch(
  ...
  return Effect.gen(function* () {
    ...
    if (
      (!ready ||
        (!hasPtyConnectTicketURL(url) && !hasPersistentPtyConnectTicketURL(url) && !hasBrowserStreamTicketURL(url))) &&
