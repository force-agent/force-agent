...
const binary = path.join(
  directory,
  `${nodeBuild ? "opencode2-node" : "labharness"}${process.platform === "win32" ? ".exe" : ""}`,
)
...
try {
  ...
  if (failure)
    errors.push(fs.readFile(path.join(root, "data", "labharness", "log", "opencode.log"), "utf8").catch(() => ""))
}
...
async function waitForRegistration() {
  const directory = path.join(root, "state", "labharness")
