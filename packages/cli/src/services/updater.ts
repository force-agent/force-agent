...
import { action, type Policy } from "@opencode-ai/util/release-version"

declare const OPENCODE_CLI_NAME: string | undefined

type Method = "npm" | "pnpm" | "bun" | "yarn" | "curl"

// force-agent overlay: the npm distribution is the unscoped `force-agent`
// package, so that is what `upgrade` detects in a global install and what it
// asks a package manager to reinstall. Pointing this at @opencode-ai/cli would
// make an upgrade replace this fork with upstream.
const packageName =
  typeof OPENCODE_CLI_NAME === "string" && OPENCODE_CLI_NAME === "opencode2-node" ? OPENCODE_CLI_NAME : "force-agent"
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const check = Effect.fn("cli.updater.check")(
      function* () {
        // force-agent overlay: self-update is OFF by default. Upstream would
        // fetch update.opencode.ai and replace this fork's binary with an
        // opencode release - on a running server that is a silent takeover of
        // the deployed artifact. Opt back in with LABHARNESS_ENABLE_AUTOUPDATE=1.
        if (!truthy("ENABLE_AUTOUPDATE"))
          return yield* Effect.logInfo("update check skipped", {
            reason: "autoupdate-disabled-by-default",
            version: OPENCODE_VERSION,
            channel: OPENCODE_CHANNEL,
          })
        if (OPENCODE_LOCAL || truthy("DISABLE_AUTOUPDATE"))
          return yield* Effect.logInfo("update check skipped", {
...
export { action, type Action, type Policy } from "@opencode-ai/util/release-version"
