...
test("concurrent service processes elect one server", async () => {
  ...
  const registration = path.join(root, "state", "labharness", "service-local.json")
  const port = await availablePort()
  const config = path.join(root, "config", "labharness", "service-local.json")
  await fs.mkdir(path.join(root, "config", "labharness"), { recursive: true })
...
test("unrelated managed port occupancy reports an actionable conflict", async () => {
  ...
  const registration = path.join(root, "state", "labharness", "service-local.json")
  await fs.mkdir(path.join(root, "config", "labharness"), { recursive: true })
  await fs.writeFile(path.join(root, "config", "labharness", "service-local.json"), JSON.stringify({ port }))
...
test("unresponsive managed port occupancy reports a bounded conflict", async () => {
  ...
  await fs.writeFile(
    path.join(root, "config", "labharness", "service-local.json"),
