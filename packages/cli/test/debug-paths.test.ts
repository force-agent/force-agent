  ...
  test("prints resolved global paths without starting a server", async () => {
    ...
    try {
      ...
      expect(paths).toMatchObject({
        home: os.homedir(),
        data: path.join(root, "data", "labharness"),
        config: path.join(root, "config", "labharness"),
        cache: path.join(root, "cache", "labharness"),
        state: path.join(root, "state", "labharness"),
        bin: path.join(root, "cache", "labharness", "bin"),
        log: path.join(root, "data", "labharness", "log"),
        repos: path.join(root, "data", "labharness", "repos"),
      })
      expect(paths.tmp).toBeTruthy()
      expect(await Bun.file(path.join(root, "state", "labharness", "service-local.json")).exists()).toBe(false)
    } finally {
