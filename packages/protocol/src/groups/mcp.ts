...
export const McpGroup = HttpApiGroup.make("server.mcp")
  ...
  .add(
    HttpApiEndpoint.get("mcp.tools", "/api/mcp/:server/tools", {
      params: { server: Schema.String },
      query: LocationQuery,
      success: Location.response(Schema.Array(Mcp.ToolSummary)),
      error: McpServerNotFoundError,
    })
      .annotateMerge(locationQueryOpenApi)
      .annotateMerge(
        OpenApi.annotations({
          identifier: "v2.mcp.tools",
          summary: "List MCP server tools",
          description: "Retrieve the tool names and descriptions exposed by one MCP server, without input schemas.",
        }),
      ),
  )
  .add(
