...
/** One-line summary of a browser tool call: the URL, the action + ref, or the reason for a handoff. */
function browserToolSubtitle(action: string, input: Record<string, unknown>): string | undefined {
  ...
  switch (action) {
    ...
    case "fetch":
      return str("url")
    case "act": {
      const verb = str("action")
      const ref = str("ref")
      if (!verb) return ref
      return ref ? `${verb} ${ref}` : verb
    }
    case "handoff":
      return str("reason")
    case "tabs":
      return str("op")
    case "eval":
      return str("expression")?.slice(0, 80)
    default:
      return undefined
  }
}
...
export function getToolInfo(
  ...
  switch (tool) {
    ...
    case "browser_network": {
      const action = tool.slice("browser_".length) as
        | "navigate" | "snapshot" | "act" | "read" | "screenshot" | "tabs" | "handoff" | "eval" | "fetch" | "network"
      return {
        icon: "window-cursor",
        title: i18n.t(`ui.tool.browser.${action}`),
        subtitle: browserToolSubtitle(action, input),
      }
    }
    case "skill":
      return {
...
export type ToolFile = { uri: string; mime: string; name?: string }
...
export interface ToolProps {
  ...
  sessionID?: string
  output?: string
  // File parts of the tool result (a browser screenshot, for instance); `output` carries the text.
  files?: ToolFile[]
  status?: string
  hideDetails?: boolean
  defaultOpen?: boolean
...
// Tool output reaches the model inside `<untrusted source="page" host="…">` with a reminder
// line; the timeline shows the page content without the envelope.
function unwrapUntrusted(text: string) {
  const match = text.match(/^<untrusted[^>]*>\n(?:[^\n]*\n)?\n?([\s\S]*?)\n?<\/untrusted>\s*$/)
  return match ? match[1] : text
}

function browserHost(metadata: Record<string, unknown>, input: Record<string, unknown>) {
  const url = typeof metadata.url === "string" ? metadata.url : typeof input.url === "string" ? input.url : undefined
...
function BrowserTool(props: ToolProps & { action: BrowserAction }) {
  const i18n = useI18n()
  const pending = () => props.status === "streaming" || props.status === "running"
  const host = createMemo(() => browserHost(props.metadata, props.input))
  const subtitle = createMemo(() => browserToolSubtitle(props.action, props.input))
  const text = createMemo(() => (props.output ? unwrapUntrusted(props.output) : ""))
  const image = createMemo(() => props.files?.find((file) => file.mime.startsWith("image/")))
  const hasBody = () => props.action !== "handoff" && props.action !== "tabs" && (!!image() || !!text())
  return (
    <BasicTool
      {...props}
      icon="window-cursor"
      ...
      trigger={
        <div data-slot="basic-tool-tool-info-structured">
          <div data-slot="basic-tool-tool-info-main">
            <span data-slot="basic-tool-tool-title">
              <TextShimmer text={i18n.t(`ui.tool.browser.${props.action}`)} active={pending()} />
            </span>
            <Show when={subtitle()}>{(value) => <span data-slot="basic-tool-tool-subtitle">{value()}</span>}</Show>
            <Show when={host()}>{(value) => <span data-slot="basic-tool-tool-arg">{value()}</span>}</Show>
          </div>
        </div>
      }
    >
      <Switch>
        ...
        <Match when={props.action === "read" && text()}>
          {(value) => (
            <div
              data-component="tool-output"
              data-scrollable
        ...
        <Match when={text()}>
          {(value) => (
            <ConsoleOutput copy={value()}>
              <span data-slot="bash-result">{value()}</span>
            </ConsoleOutput>
          )}
        </Match>
      </Switch>
...
for (const action of BROWSER_ACTIONS)
  ToolRegistry.register({
    ...
    render: (props) => <BrowserTool {...props} action={action} />,
  })
