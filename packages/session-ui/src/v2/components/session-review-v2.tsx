...
function reviewFileIndex(files: string[], active: string | undefined) {
...
/**
 * Previous/next file arrows plus the ←/→ shortcut they advertise. The
 * shortcut only runs while the component is mounted, never while typing in an
 * input or comment editor, and never while a tab strip owns the arrow keys.
 */
export function SessionReviewV2FileNav(props: {
  files: string[]
  activeFile?: string
  onSelectFile: (file: string) => void
}) {
  ...
  const fileIndex = () => reviewFileIndex(props.files, props.activeFile)
  const prev = () => {
    if (!canCycle()) return
  ...
  makeEventListener(document, "keydown", (event) => {
    ...
    // A hidden workspace panel stays mounted (`hidden` + `inert`); its nav
    // must not claim the arrows from the visible one.
    if (root?.closest("[inert]")) return
    const target = event.target
    if (
      target instanceof HTMLElement &&
      (target.isContentEditable || target.closest("input, textarea, select, [role='tab']"))
    )
      return
    if (!canCycle()) return
...
/** Expand/collapse unchanged lines and, when supported, unified/split layout. */
export function SessionReviewV2DiffControls(props: {
  expandMode: SessionReviewExpandMode
  onExpandModeChange: (mode: SessionReviewExpandMode) => void
  diffStyle: SessionReviewDiffStyle
  onDiffStyleChange?: (style: SessionReviewDiffStyle) => void
...
export function SessionReviewV2(props: SessionReviewV2Props) {
  ...
  const toolbarStart = () => (
    <>
      ...
      <SessionReviewV2FileNav files={props.files} activeFile={props.activeFile} onSelectFile={props.onSelectFile} />
    </>
  )
  ...
  const toolbarEnd = () => (
    <SessionReviewV2DiffControls
      ...
      onExpandModeChange={props.onExpandModeChange}
