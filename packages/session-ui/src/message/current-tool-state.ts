...
export function currentToolFiles(tool: SessionMessageAssistantTool) {
  if (!("content" in tool.state) || !tool.state.content) return undefined
  const files = tool.state.content.flatMap((item) =>
    item.type === "file" ? [{ uri: item.uri, mime: item.mime, name: item.name ?? undefined }] : [],
  )
