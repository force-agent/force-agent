...
const EndpointUpdateGet = (raw: RawClient["server.update"]) => (input?: UpdateGetInput) =>
  preserveEffect<UpdateGetOutput>()(
    raw["update.get"]({ query: { refresh: input?.["refresh"] } }).pipe(Effect.mapError(mapClientError)),
  )

const EndpointUpdateApply = (raw: RawClient["server.update"]) => (input?: UpdateApplyInput) =>
  preserveEffect<UpdateApplyOutput>()(
    raw["update.apply"]({ payload: { version: input?.["version"] } }).pipe(Effect.mapError(mapClientError)),
  )

const adaptGroupUpdate = (raw: RawClient["server.update"]) => ({
  get: EndpointUpdateGet(raw),
  apply: EndpointUpdateApply(raw),
})
...
const EndpointMcpTools = (raw: RawClient["server.mcp"]) => (input: McpToolsInput) =>
  preserveEffect<McpToolsOutput>()(
    raw["mcp.tools"]({ params: { server: input["server"] }, query: { location: input["location"] } }).pipe(
      Effect.mapError(mapClientError),
    ),
  )
...
const EndpointCapabilityList = (raw: RawClient["server.capability"]) => (input?: CapabilityListInput) =>
  preserveEffect<CapabilityListOutput>()(
    raw["capability.list"]({ query: { location: input?.["location"], agent: input?.["agent"] } }).pipe(
      Effect.mapError(mapClientError),
    ),
  )

const EndpointCapabilityRefresh = (raw: RawClient["server.capability"]) => (input?: CapabilityRefreshInput) =>
  preserveEffect<CapabilityRefreshOutput>()(
    raw["capability.refresh"]({ query: { location: input?.["location"] } }).pipe(Effect.mapError(mapClientError)),
  )

const adaptGroupCapability = (raw: RawClient["server.capability"]) => ({
  list: EndpointCapabilityList(raw),
  refresh: EndpointCapabilityRefresh(raw),
})

const EndpointRoutineList = (raw: RawClient["server.routine"]) => (input?: RoutineListInput) =>
  preserveEffect<RoutineListOutput>()(
    raw["routine.list"]({ query: { location: input?.["location"] } }).pipe(Effect.mapError(mapClientError)),
  )

const EndpointRoutineCreate = (raw: RawClient["server.routine"]) => (input: RoutineCreateInput) =>
  preserveEffect<RoutineCreateOutput>()(
    raw["routine.create"]({
      query: { location: input["location"] },
      payload: {
        name: input["name"],
...
const EndpointRoutineUpdate = (raw: RawClient["server.routine"]) => (input: RoutineUpdateInput) =>
  preserveEffect<RoutineUpdateOutput>()(
    raw["routine.update"]({
      params: { id: input["id"] },
      query: { location: input["location"] },
      payload: {
...
const EndpointRoutineRemove = (raw: RawClient["server.routine"]) => (input: RoutineRemoveInput) =>
  preserveEffect<RoutineRemoveOutput>()(
    raw["routine.remove"]({ params: { id: input["id"] }, query: { location: input["location"] } }).pipe(
      Effect.mapError(mapClientError),
    ),
  )

const EndpointRoutineRun = (raw: RawClient["server.routine"]) => (input: RoutineRunInput) =>
  preserveEffect<RoutineRunOutput>()(
    raw["routine.run"]({ params: { id: input["id"] }, query: { location: input["location"] } }).pipe(
      Effect.mapError(mapClientError),
    ),
  )

const EndpointRoutineRuns = (raw: RawClient["server.routine"]) => (input: RoutineRunsInput) =>
  preserveEffect<RoutineRunsOutput>()(
    raw["routine.runs"]({
      params: { id: input["id"] },
...
const adaptGroupRoutine = (raw: RawClient["server.routine"]) => ({
  list: EndpointRoutineList(raw),
  create: EndpointRoutineCreate(raw),
  update: EndpointRoutineUpdate(raw),
  remove: EndpointRoutineRemove(raw),
  run: EndpointRoutineRun(raw),
  runs: EndpointRoutineRuns(raw),
})
...
const EndpointBrowserState = (raw: RawClient["server.browser"]) => (input?: BrowserStateInput) =>
  preserveEffect<BrowserStateOutput>()(
    raw["browser.state"]({ query: { location: input?.["location"] } }).pipe(Effect.mapError(mapClientError)),
  )

const EndpointBrowserTabOpen = (raw: RawClient["server.browser"]) => (input?: BrowserTabOpenInput) =>
  preserveEffect<BrowserTabOpenOutput>()(
    raw["browser.tab.open"]({ query: { location: input?.["location"] }, payload: { url: input?.["url"] } }).pipe(
      Effect.mapError(mapClientError),
    ),
  )

const EndpointBrowserTabClose = (raw: RawClient["server.browser"]) => (input: BrowserTabCloseInput) =>
  preserveEffect<BrowserTabCloseOutput>()(
    raw["browser.tab.close"]({ params: { tabID: input["tabID"] }, query: { location: input["location"] } }).pipe(
      Effect.mapError(mapClientError),
    ),
  )

const EndpointBrowserTabActivate = (raw: RawClient["server.browser"]) => (input: BrowserTabActivateInput) =>
  preserveEffect<BrowserTabActivateOutput>()(
    raw["browser.tab.activate"]({ params: { tabID: input["tabID"] }, query: { location: input["location"] } }).pipe(
      Effect.mapError(mapClientError),
    ),
  )

const EndpointBrowserTabBack = (raw: RawClient["server.browser"]) => (input: BrowserTabBackInput) =>
  preserveEffect<BrowserTabBackOutput>()(
    raw["browser.tab.back"]({ params: { tabID: input["tabID"] }, query: { location: input["location"] } }).pipe(
      Effect.mapError(mapClientError),
    ),
  )

const EndpointBrowserTabReload = (raw: RawClient["server.browser"]) => (input: BrowserTabReloadInput) =>
  preserveEffect<BrowserTabReloadOutput>()(
    raw["browser.tab.reload"]({ params: { tabID: input["tabID"] }, query: { location: input["location"] } }).pipe(
      Effect.mapError(mapClientError),
    ),
  )

const EndpointBrowserTabTicket = (raw: RawClient["server.browser"]) => (input: BrowserTabTicketInput) =>
  preserveEffect<BrowserTabTicketOutput>()(
    raw["browser.tab.ticket"]({
...
const EndpointBrowserTabStream = (raw: RawClient["server.browser"]) => (input: BrowserTabStreamInput) =>
  preserveEffect<BrowserTabStreamOutput>()(
    raw["browser.tab.stream"]({ params: { tabID: input["tabID"] } }).pipe(Effect.mapError(mapClientError)),
  )

const EndpointBrowserNavigate = (raw: RawClient["server.browser"]) => (input: BrowserNavigateInput) =>
  preserveEffect<BrowserNavigateOutput>()(
    raw["browser.navigate"]({
      query: { location: input["location"] },
      payload: { url: input["url"], tab: input["tab"], wait: input["wait"] },
    }).pipe(Effect.mapError(mapClientError)),
  )

const EndpointBrowserControl = (raw: RawClient["server.browser"]) => (input: BrowserControlInput) =>
  preserveEffect<BrowserControlOutput>()(
    raw["browser.control"]({ query: { location: input["location"] }, payload: { owner: input["owner"] } }).pipe(
      Effect.mapError(mapClientError),
    ),
  )

const EndpointBrowserSnapshot = (raw: RawClient["server.browser"]) => (input?: BrowserSnapshotInput) =>
  preserveEffect<BrowserSnapshotOutput>()(
    raw["browser.snapshot"]({
      query: { location: input?.["location"] },
      payload: { tab: input?.["tab"], mode: input?.["mode"], maxNodes: input?.["maxNodes"] },
    }).pipe(Effect.mapError(mapClientError)),
  )

const EndpointBrowserRead = (raw: RawClient["server.browser"]) => (input?: BrowserReadInput) =>
  preserveEffect<BrowserReadOutput>()(
    raw["browser.read"]({
      query: { location: input?.["location"] },
      payload: { tab: input?.["tab"], selector: input?.["selector"], page: input?.["page"] },
    }).pipe(Effect.mapError(mapClientError)),
  )

const EndpointBrowserAct = (raw: RawClient["server.browser"]) => (input: BrowserActInput) =>
  preserveEffect<BrowserActOutput>()(
    raw["browser.act"]({
      query: { location: input["location"] },
      payload: {
        tab: input["tab"],
...
const EndpointBrowserScreenshot = (raw: RawClient["server.browser"]) => (input?: BrowserScreenshotInput) =>
  preserveEffect<BrowserScreenshotOutput>()(
    raw["browser.screenshot"]({
      query: { location: input?.["location"] },
      payload: { tab: input?.["tab"], ref: input?.["ref"], fullPage: input?.["fullPage"] },
    }).pipe(Effect.mapError(mapClientError)),
  )

const EndpointBrowserNetwork = (raw: RawClient["server.browser"]) => (input?: BrowserNetworkInput) =>
  preserveEffect<BrowserNetworkOutput>()(
    raw["browser.network"]({
      query: { location: input?.["location"] },
      payload: {
        tab: input?.["tab"],
...
const EndpointBrowserFetch = (raw: RawClient["server.browser"]) => (input: BrowserFetchInput) =>
  preserveEffect<BrowserFetchOutput>()(
    raw["browser.fetch"]({
      query: { location: input["location"] },
      payload: {
        tab: input["tab"],
...
const EndpointBrowserEval = (raw: RawClient["server.browser"]) => (input: BrowserEvalInput) =>
  preserveEffect<BrowserEvalOutput>()(
    raw["browser.eval"]({
      query: { location: input["location"] },
      payload: { tab: input["tab"], expression: input["expression"], timeoutMs: input["timeoutMs"] },
    }).pipe(Effect.mapError(mapClientError)),
  )

const EndpointBrowserProvider = (raw: RawClient["server.browser"]) => () =>
  preserveEffect<BrowserProviderOutput>()(raw["browser.provider"]({}).pipe(Effect.mapError(mapClientError)))

const adaptGroupBrowser = (raw: RawClient["server.browser"]) => ({
  state: EndpointBrowserState(raw),
  tab: {
    open: EndpointBrowserTabOpen(raw),
    close: EndpointBrowserTabClose(raw),
    activate: EndpointBrowserTabActivate(raw),
    back: EndpointBrowserTabBack(raw),
    reload: EndpointBrowserTabReload(raw),
    ticket: EndpointBrowserTabTicket(raw),
    stream: EndpointBrowserTabStream(raw),
  },
  navigate: EndpointBrowserNavigate(raw),
  control: EndpointBrowserControl(raw),
  snapshot: EndpointBrowserSnapshot(raw),
  read: EndpointBrowserRead(raw),
  act: EndpointBrowserAct(raw),
  screenshot: EndpointBrowserScreenshot(raw),
  network: EndpointBrowserNetwork(raw),
  fetch: EndpointBrowserFetch(raw),
  eval: EndpointBrowserEval(raw),
  provider: EndpointBrowserProvider(raw),
})

const adaptClient = (raw: RawClient) => ({
  health: adaptGroupHealth(raw["server.health"]),
  server: adaptGroupServer(raw["server.server"]),
  update: adaptGroupUpdate(raw["server.update"]),
  ...
  capability: adaptGroupCapability(raw["server.capability"]),
  routine: adaptGroupRoutine(raw["server.routine"]),
  ...
  browser: adaptGroupBrowser(raw["server.browser"]),
})
