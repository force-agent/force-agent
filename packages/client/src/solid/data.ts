...
export type CreateDataInput = {
  ...
  readonly connection?: {
    readonly status: () => "connected" | "connecting" | "reconnecting" | "unauthorized"
  }
}
  ...
  function handleEvent(event: OpenCodeEvent) {
    ...
    switch (event.type) {
      ...
      case "routine.run.finished":
        result.location.routine.invalidate(location)
        void result.location.routine.sync(location)
        break
    ...
    location: {
      ...
      async sync(ref?: LocationRef) {
        ...
        await Promise.all([
          ...
          result.location.routine.sync(location),
      ...
      routine: {
        list(location?: LocationRef) {
          return store.location[locationKey(location ?? defaultLocation())]?.routine
        },
        sync(ref?: LocationRef) {
          const id = locationKey(ref ?? defaultLocation())
          return sync.run(`location.routine:${id}`, async () => {
            const response = await api().routine.list({ location: locationQuery(ref ?? defaultLocation()) })
            const key = locationKey(response.location)
            setStore("location", key, { ...store.location[key], routine: response.data })
          })
        },
        invalidate(ref?: LocationRef) {
          sync.invalidate(`location.routine:${locationKey(ref ?? defaultLocation())}`)
        },
      },
    },
  }
