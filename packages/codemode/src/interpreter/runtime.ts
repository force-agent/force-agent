...
import { dateMethods, dateStatics, hostNow } from "../stdlib/date.js"
...
export class Interpreter<R> {
  ...
  private constructDate(args: Array<unknown>, node: AstNode): Effect.Effect<CodeModeDate, unknown, R> {
    // power-agent overlay: the zero-argument constructor is a host-clock read, so it goes through
    // the same guard as `Date.now()` (see stdlib/date.ts). Delegation only — no policy here.
    if (args.length === 0) return Effect.succeed(new CodeModeDate(hostNow("new Date()", node)))
    if (args.length === 1) {
      const arg = args[0]
  ...
  private invokeCallable(
    ...
    return Effect.gen(function* () {
      ...
      if (callable instanceof GlobalNamespace) {
        ...
        if (callable.name === "Date") return new Date(hostNow("Date()", node)).toISOString()
