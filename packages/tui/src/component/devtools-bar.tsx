          ...
          fg={
            panel() === "server"
              ...
              : serverIndicator().state === "connected"
                ...
                : serverIndicator().state === "disconnected" || serverIndicator().state === "unauthorized"
...
export function connectionIndicator(
  status: "connected" | "connecting" | "reconnecting" | "unauthorized",
  attempt: number,
) {
  if (status === "connected") return { state: "connected" as const, icon: "✓" }
  // A rejected credential is not a flaky link: it stays broken until someone
  // fixes it, so it gets its own mark instead of a spinner that never settles.
  if (status === "unauthorized") return { state: "unauthorized" as const, icon: "⚠" }
