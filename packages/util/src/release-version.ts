// Pure semver release comparison shared by the CLI background updater and the
...
/** True when both versions parse and name different releases (major bumps included). */
export function differentRelease(current: string, latest: string): boolean {
  const currentVersion = parseReleaseVersion(current)
  const latestVersion = parseReleaseVersion(latest)
  if (!currentVersion || !latestVersion) return false
  return !sameRelease(currentVersion, latestVersion)
}
