...
const stderrLogger = Logger.make((options) => {
  // force-agent overlay: read through the branded helper so LABHARNESS_PRINT_LOGS (and the legacy
  // LABFY_PRINT_LOGS, POWER_PRINT_LOGS) works.
  if (env("PRINT_LOGS") !== "1") return
  process.stderr.write(formatter().log(options) + "\n")
})

export function minimumLogLevel() {
  // force-agent overlay: LABHARNESS_LOG_LEVEL, falling back to LABFY_LOG_LEVEL, POWER_LOG_LEVEL then OPENCODE_LOG_LEVEL.
