...
/**
 * force-agent overlay: catalog source read from the branded env helper whenever
 * the caller did not pass one explicitly.
 *
 * `LABHARNESS_MODELS_URL` used to be honored only where the CLI *builds* these
 * options (`ServerProcess.models()`), so every other consumer — `ModelsDev.node`
 * itself, the simulation worker, the stats app, any embedder of the server —
 * went on calling the default host with the operator's override sitting unread
 * in the environment. Resolving it here makes the env the floor for everyone.
 *
 * Precedence: explicit option > `LABHARNESS_*` > `LABFY_*` > `POWER_*` > `OPENCODE_*` > default. `??` rather
 * than `||` on purpose: `url: ""` is an explicit "use the default host" that the
 * callers below already rely on, and it must not fall through to the env.
 */
function resolveOptions(options?: Options) {
  return {
    url: options?.url ?? env("MODELS_URL"),
    file: options?.file ?? env("MODELS_PATH"),
...
export const layer = (options?: Options) =>
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const source = resolved.url || defaultSource
      ...
      const loadFromFile = resolved.file
        ? fs.readJson(resolved.file).pipe(
      ...
      const populate = Effect.gen(function* () {
        ...
        const cached = resolved.file ? undefined : yield* loadFromCache()
        ...
        const catalog = yield* lock.withPermit(
          Effect.gen(function* () {
            const stored = resolved.file ? undefined : yield* loadFromCache()
