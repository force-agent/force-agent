...
export interface Interface extends State.Transformable<Draft> {
  ...
  readonly tools: (server?: ServerName | string) => Effect.Effect<Tool[]>
  readonly config: (server: ServerName | string) => Effect.Effect<Mcp.ServerConfig | undefined>
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      return Service.of({
        ...
        tools: Effect.fn("MCP.tools")(function* (server) {
          if (server !== undefined) {
            const entry = entries.get(ServerName.make(server))
            if (!entry) return []
            yield* entry.startup.await
            return (entry.tools ?? []).toSorted((a, b) => a.name.localeCompare(b.name))
          }
          yield* whenAllReady
        ...
        config: Effect.fn("MCP.config")(function* (server) {
          return entries.get(ServerName.make(server))?.config
        }),
