...
import { SandboxUnavailableError, resolveBwrap, sandboxArgs } from "./shell/sandbox.js"

// Sandbox opt-in: LABHARNESS_SANDBOX=1 wraps every shell in bubblewrap (see shell/sandbox.ts).
// Fail closed: a missing bwrap is a configuration error, never a silent fallback.
function sandboxEnabled(): boolean {
  return Env.truthy("SANDBOX")
}
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const create = Effect.fn("Shell.create")(function* <E = never, R = never>(
        ...
        let spawnShell = invocation.shell
        let spawnArgs = args
        if (sandboxEnabled()) {
          const bwrap = process.platform === "linux" ? resolveBwrap() : undefined
          if (!bwrap) return yield* Effect.die(new SandboxUnavailableError())
          spawnShell = bwrap
          spawnArgs = sandboxArgs({ cwd: invocation.cwd, shell: invocation.shell, args })
        }
          ...
          Effect.scoped(
            Effect.gen(function* () {
              const handle = yield* environment.spawner
                .spawn(
                  ChildProcess.make(spawnShell, spawnArgs, {
                    cwd: invocation.cwd,
                    env: invocation.env,
                    stdin: "ignore",
