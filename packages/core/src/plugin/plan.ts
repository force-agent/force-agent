  ...
  effect: Effect.fn(function* (ctx) {
    ...
    yield* ctx.agent.transform((draft) => {
      draft.update(plan, (item) => {
        ...
        // Plan mode may look at pages (`browser.read`) but never drive them.
        item.permissions.push({ action: "browser", resource: "*", effect: "deny" })
        item.permissions.push({ action: "browser.eval", resource: "*", effect: "deny" })
        item.permissions.push({ action: "browser.fetch", resource: "*", effect: "deny" })
