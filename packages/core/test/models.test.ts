...
describe("ModelsDev Service", () => {
  ...
  // power-agent overlay: the catalog source falls back to the branded env helper
  // when the caller passes no `url`. Before this, only `ServerProcess.models()`
  // read POWER_MODELS_URL, so any layer built outside the CLI (ModelsDev.node,
  // the simulation worker, an embedder) silently kept calling the default host.
  // Every case here clears both spellings first: the suite runs in a mostly
  // inherited environment.
  const modelsEnv = {
    POWER_MODELS_URL: undefined,
    OPENCODE_MODELS_URL: undefined,
    POWER_MODELS_PATH: undefined,
    OPENCODE_MODELS_PATH: undefined,
    POWER_DISABLE_MODELS_FETCH: undefined,
    OPENCODE_DISABLE_MODELS_FETCH: undefined,
  }

  it.live("resolves the catalog host from POWER_MODELS_URL when no url option is passed", () =>
    withEnv({ ...modelsEnv, POWER_MODELS_URL: "https://branded.test" }, () =>
      Effect.gen(function* () {
        const cache = makeCache()
        const state = yield* Ref.make(initialState)
        yield* ModelsDev.Service.use((service) => service.get()).pipe(
          Effect.provide(buildLayer(state, cache, { fetch: true, snapshot: false })),
        )
        expect((yield* Ref.get(state)).calls[0]?.url).toBe("https://branded.test/api.json")
      }),
    ),
  )

  it.live("still honors the upstream OPENCODE_MODELS_URL on its own", () =>
    withEnv({ ...modelsEnv, OPENCODE_MODELS_URL: "https://legacy.test" }, () =>
      Effect.gen(function* () {
        const cache = makeCache()
        const state = yield* Ref.make(initialState)
        yield* ModelsDev.Service.use((service) => service.get()).pipe(
        ...
        expect((yield* Ref.get(state)).calls[0]?.url).toBe("https://legacy.test/api.json")
      }),
    ),
  )

  it.live("the branded env outranks the upstream one", () =>
    withEnv(
      { ...modelsEnv, POWER_MODELS_URL: "https://branded.test", OPENCODE_MODELS_URL: "https://legacy.test" },
      () =>
        Effect.gen(function* () {
          const cache = makeCache()
          const state = yield* Ref.make(initialState)
  ...
  it.live("an explicit url option beats both env spellings", () =>
    withEnv(
      { ...modelsEnv, POWER_MODELS_URL: "https://branded.test", OPENCODE_MODELS_URL: "https://legacy.test" },
      () =>
        Effect.gen(function* () {
          ...
          const state = yield* Ref.make(initialState)
          yield* ModelsDev.Service.use((service) => service.get()).pipe(
            Effect.provide(buildLayer(state, cache, { url: "https://explicit.test", fetch: true, snapshot: false })),
          )
          expect((yield* Ref.get(state)).calls[0]?.url).toBe("https://explicit.test/api.json")
        }),
    ),
  )

  it.live("an explicit empty url still means the default host, not the env", () =>
    withEnv({ ...modelsEnv, POWER_MODELS_URL: "https://branded.test" }, () =>
      Effect.gen(function* () {
        const cache = makeCache()
        const state = yield* Ref.make(initialState)
  ...
  it.live("reads the catalog file named by POWER_MODELS_PATH when no file option is passed", () =>
    withTempDir((tmp) =>
      Effect.gen(function* () {
        const file = path.join(tmp.path, "catalog.json")
        yield* Effect.promise(() => fs.writeFile(file, JSON.stringify(fixture2)))
        return yield* withEnv({ ...modelsEnv, POWER_MODELS_PATH: file }, () =>
          Effect.gen(function* () {
            const cache = makeCache()
            const state = yield* Ref.make(initialState)
            // KV holds a different catalog: the file must win, exactly as an
            // explicit `file` option does.
            writeCache(cache, fixture)
  ...
  it.live("POWER_DISABLE_MODELS_FETCH turns the background fetch off without a fetch option", () =>
    withEnv({ ...modelsEnv, POWER_DISABLE_MODELS_FETCH: "true" }, () =>
      Effect.gen(function* () {
        const cache = makeCache()
        const state = yield* Ref.make(initialState)
        const result = yield* ModelsDev.Service.use((service) => service.get()).pipe(
          Effect.provide(buildLayer(state, cache, { snapshot: false })),
        )
        expect(result).toEqual([])
        expect((yield* Ref.get(state)).calls).toEqual([])
      }),
