...
describe("Core test environment", () => {
  test("isolates global home and XDG roots", () => {
    ...
    expect(Global.Path.config).toBe(path.join(home, ".config", "labharness"))
    expect(Global.Path.data).toBe(path.join(home, ".local", "share", "labharness"))
    expect(Global.Path.cache).toBe(path.join(home, ".cache", "labharness"))
    expect(Global.Path.state).toBe(path.join(home, ".local", "state", "labharness"))
    expect(os.tmpdir()).toBe(path.join(home, "tmp"))
    expect(process.env.LABHARNESS_CONFIG_DIR).toBe(Global.Path.config)
