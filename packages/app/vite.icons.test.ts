...
async function check(channel: string, read: (path: string) => Promise<Uint8Array>) {
  ...
  // Anonymous by default, the manifest fetch 401s behind the Basic credential
  // `force web` serves the UI under, and the browser answers a 401 by reopening
  // the password prompt — forever, since typing the right one changes nothing.
