...
async function installSessionSwitchProbe(
  ...
  await page.evaluate(({ destinationIDs, sourceIDs, lastID, requiredPartID, requireBottomAnchor, href }) => {
    ...
    let running = true
    // The Files tab absorbed the review pane: its sidebar tree and the diff
    // preview of the open file are the nodes a session switch must not replace.
    const reviewLevels: Record<string, string> = {
      panel: "#review-panel",
      tabs: '#review-panel [data-component="tabs"]',
      files: "#session-workspace-tabpanel-files",
      sidebar: '#review-panel [data-slot="session-review-v2-sidebar"]',
