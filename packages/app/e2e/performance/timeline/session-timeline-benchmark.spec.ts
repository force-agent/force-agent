...
async function measureReviewPaneLoad(page: Page, file: string) {
  // The Files tab absorbed the review pane. The summary popover's "Changed
  // files" row opens it in git mode (the mocked /vcs/diff data) with the first
  // changed file selected — the one click the old toggle used to be.
  await page.getByRole("button", { name: "Session details" }).click()
  const open = page
    .locator('[data-component="session-summary-panel"]')
    .getByRole("button", { name: /Changed files?|No changes|Loading changes/ })
