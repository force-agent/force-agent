...
// The Files tab absorbed the review pane: opening it lists the changed files
// in its sidebar tree (git mode by default).
async function openReviewPane(page: Page) {
  ...
  await expect(
    page.locator('#review-panel [data-slot="session-review-v2-sidebar"] button[data-slot="file-tree-v2-row"]', {
      hasText: "generated-000.ts",
    }),
  ).toBeVisible()
}
