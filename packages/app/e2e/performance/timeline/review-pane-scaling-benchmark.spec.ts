...
async function measureReviewPaneLoad(page: Page, input: { expectedFile: string; expectedRows: number }) {
  // The Files tab absorbed the review pane. The one click that opens it with
  // the first changed file selected (what the old toggle did) is the summary
  // popover's "Changed files" row.
