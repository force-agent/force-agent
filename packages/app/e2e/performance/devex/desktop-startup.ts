...
async function waitForHome(page: Page, mark: (name: Milestone) => void) {
  ...
  // The home lost its "Projects" column (ui-round-3 task-01); the nav sidebar is
  // the only project navigation now, so "add project" lives in its title row.
  ...
  const addProject = nav.locator('button[data-action="nav-sidebar-add-project"]')
