...
export interface MockServerConfig {
  ...
  /** Skills the workspace exposes; drives the right-hand Skills/MCP strip. */
  skills?: unknown[]
  /** MCP servers the workspace exposes; drives the right-hand Skills/MCP strip. */
  mcps?: unknown[]
  /** Shared browser state; defaults to no browser running. */
  browser?: unknown
  ...
  /** `/api/health` body; read on every call so a test can change the pid/version mid-flight. */
  health?: () => unknown
  /** `GET /api/update` status (`SelfUpdate.Status`); defaults to "nothing available, cannot apply". */
  update?: () => unknown
  /** `POST /api/update/apply`: return the accepted status (202). */
  onUpdateApply?: (input: { body: unknown }) => unknown
}
...
function mockHandlers(config: MockServerConfig, state: { cursors: Map<string, string>; nextCursor: number }) {
  ...
  return HttpApiBuilder.group(MockApi, "mock", (handlers) =>
    handlers
      ...
      .handleAll({
        health: () => Effect.sync(() => config.health?.() ?? { healthy: true, version: "2.0.0", pid: 1 }),
        update: () => Effect.sync(() => config.update?.() ?? defaultUpdateStatus),
        updateApply: (ctx) =>
          Effect.sync(() => config.onUpdateApply?.({ body: ctx.payload }) ?? config.update?.() ?? defaultUpdateStatus),
        reference: () =>
          Effect.succeed({
            location: {
        ...
        mcp: () => Effect.succeed({ location: location(config), data: config.mcps ?? [] }),
        ...
        browserState: () =>
          Effect.succeed({
            location: location(config),
            data: config.browser ?? { running: false, profile: "e2e", control: "idle", tabs: [] },
          }),
        sessionList: (ctx) => {
