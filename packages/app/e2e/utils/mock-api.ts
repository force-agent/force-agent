...
const Group = HttpApiGroup.make("mock")
  ...
  .add(HttpApiEndpoint.get("update", "/api/update", { success: Json }))
  .add(
    HttpApiEndpoint.post("updateApply", "/api/update/apply", {
      payload: JsonPayload,
      success: Json.pipe(HttpApiSchema.status(202)),
    }),
  )
  .add(
  ...
  .add(HttpApiEndpoint.get("browserState", "/api/browser", { success: Json }))
  .add(
