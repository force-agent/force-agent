...
test.describe("smoke: session timeline", () => {
  ...
  test("renders seeded timeline in order while paging through history", async ({ page }) => {
    ...
    await selectHomeProject(page, fixture.directory)
...
async function configureSmokePage(page: Page, directory: string) {
  // Merge, do not overwrite: useLegacyTabStrip seeds appearance.tabLayout into the
  // same key from an earlier init script.
  await page.addInitScript(() => {
...
async function selectHomeProject(page: Page, directory: string) {
  // The home lost its project column (round 3): the project is picked by seeding the
  // persisted selection, which is what clicking a project in the nav sidebar stores.
  await page.addInitScript((directory) => {
    localStorage.setItem(
      "opencode.global.dat:server",
      JSON.stringify({
  ...
  await page.goto("/")
  await expectAppVisible(page.locator('[data-action="home-new-session"]'))
