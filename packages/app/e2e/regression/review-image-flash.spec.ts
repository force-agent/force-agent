...
// The Files tab absorbed the review: a changed image opens through the same
// review preview it did in the retired Diffs tab.
const PANEL = "#session-workspace-tabpanel-files"

test("clicking an image file in the Files tab does not blank the panel", async ({ page }) => {
...
async function openReview(page: Page) {
  ...
  await expectAppVisible(page.locator(`${PANEL} [data-slot="session-review-v2-sidebar"]`))
  await expectAppVisible(page.locator(PANEL).getByRole("button", { name: /preview\.png/ }))
}
...
async function installReviewFlashProbe(page: Page) {
  await page.evaluate(() => {
    ...
    const sample = () => {
      const panel = document.querySelector<HTMLElement>("#session-workspace-tabpanel-files")
