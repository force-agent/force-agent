...
test("navigates to a subagent child session missing from the session list", async ({ page }) => {
  ...
  // The child page is a full session screen: its workspace tab bar (with the
  // Files tab that absorbed the review) is mounted.
  await expect(page.locator('[data-slot="session-workspace-tabs-bar"] [role="tab"][data-tab="files"]')).toHaveCount(1)
})
...
test("keeps the parent tab selected while a loaded child session resolves", async ({ page }) => {
  ...
  // The topbar Home button is gone; in the legacy strip the keybind is the way home.
