...
test("opens and searches project files inline", async ({ page }) => {
  ...
  // The Files tab absorbed the review: its sidebar (tree + filter) is always
  // open, so opening a file never hides it, and the header is the changes-mode
  // select rather than the project name.
  ...
  await expect(panel.getByRole("tab", { name: "Open file", selected: true })).toBeVisible()
  // Leave the tab and come back: the file tabs survive the round trip.
  await contextTab.click()
  await expect(contextTab).toHaveAttribute("aria-selected", "true")
  await filesTab.click()
