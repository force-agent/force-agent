  ...
  return {
    ...
    project: {
      ...
      add: (conn: ServerConnection.Any, directories: string[]) => {
        ...
        if (!directory) return
        addProjectsToContext(global.ensureServerCtx(conn), directories)
...
/** Register directories as projects on a server context (shared by home and the nav sidebar). */
export function addProjectsToContext(ctx: ServerCtx, directories: string[]) {
  const directory = directories[0]
  if (!directory) return
