...
export const { use: useFile, provider: FileProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    const load = (input: string, options?: { force?: boolean }) => {
      ...
      if (!options?.force) {
        const cached = getFileContentCache(directory, file)
        if (cached) {
          setLoaded(file, cached)
          touchFileContent(file, approxBytes(cached))
      ...
      const promise = serverSDK.api.file
        ...
        .then((data) => {
          ...
          const content = createFileContent(file, data)
          setLoaded(file, content)

          if (!content) return
          setFileContentCache(directory, file, content)
    ...
    createEffect(() => {
      const stop = sdk().event.on("filesystem.changed", (event) => {
        const normalized = path.normalize(event.data.file)
        if (normalized) invalidateFileContentCache(scope(), normalized)
