...
async function blobReference(file: File) {
  // `crypto.subtle` is undefined outside a secure context, which is exactly what the `web`
  // command produces when it binds a LAN address over plain HTTP. The id only has to be
  // stable-per-content within a session, so fall back the same way the draft store does
  // (see runtime/persistence/drafts.ts) rather than throwing on every attachment.
