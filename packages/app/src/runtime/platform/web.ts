...
import { createWebUpdater } from "./web-updater"
...
export function createWebPlatform(
  version: string,
  options?: {
    /** Basic credential (base64 `user:password`) the page was opened with, when any. */
  ...
  const platform: Platform = {
    ...
    version,
    // Self-update of the local `force web` server: install, restart, reload once it is back.
    updater: createWebUpdater({
      baseUrl: currentServerUrl,
      authorization: options?.authorization,
      serviceWorker:
        typeof navigator === "object" && "serviceWorker" in navigator ? navigator.serviceWorker : undefined,
    }),
    openExternal(value) {
