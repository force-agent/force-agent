...
export type BrowserViewLocation = { directory: string; workspaceID?: string }
export type BrowserViewBounds = { x: number; y: number; width: number; height: number }

/** Native browser views hosted by the desktop window (desktop only) */
export type BrowserViewPlatform = {
  /** Where the browser panel sits, in CSS pixels of the app viewport */
  setBounds(location: BrowserViewLocation, bounds: BrowserViewBounds): Promise<void>
  /** Whether the panel is on screen; the first call also binds the window as host for the location */
  setVisible(location: BrowserViewLocation, visible: boolean): Promise<void>
  /** Move keyboard focus into the native view */
  focus(location: BrowserViewLocation): Promise<void>
}
...
type PlatformBase = {
  ...
  /** Native browser views for the shared agent browser (desktop only) */
  browserView?: BrowserViewPlatform
}
