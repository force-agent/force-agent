      ...
      >
        <div class="absolute inset-x-0 top-[25.375%] flex justify-center px-6">
          <div class={NEW_SESSION_CONTENT_WIDTH}>
            {/* Text wordmark: the fork has no SVG logotype yet, and the upstream one spells "opencode". */}
            <div
              data-slot="new-session-wordmark"
              class="w-full select-none text-center font-mono text-[clamp(40px,7vw,88px)] font-semibold leading-none tracking-[-0.04em] text-v2-background-bg-inverse opacity-60"
              aria-hidden="true"
            >
              force agent
            </div>
...
/**
 * Rendered in place at the page's top-right corner: the TitlebarRight slot only
 * has a mount inside the session workspace tab bar, which this draft page never
 * renders, so portaling here would drop the trigger on every platform.
 */
export function NewSessionStatus(props: { visible: boolean }) {
  ...
  return (
    <Show when={props.visible}>
      <div
        data-slot="new-session-status"
        class="absolute right-3 top-[calc(var(--shell-top-inset,8px)+4px)] z-10 flex shrink-0 items-center"
      >
