...
import { useUpdaterToasts } from "@/shell/updates/action"

const DebugBar = lazy(() => import("@/shell/debug/debug-bar").then((module) => ({ default: module.DebugBar })))
const NavSidebar = lazy(() => import("@/shell/sidebar/nav-sidebar").then((module) => ({ default: module.NavSidebar })))
...
export default function Layout(props: ParentProps) {
  ...
  const sidebarEnabled = () => preferences.appearance.tabLayout() !== "horizontal" && !mobile()
  // Desktop keeps the header as the window drag region (and the traffic lights' room);
  // the web build has nothing left to show there once the nav sidebar is on.
  const titlebarHidden = () => platform.platform !== "desktop" && sidebarEnabled()
  ...
  return (
    <TitlebarRightProvider>
      ...
      >
        <Titlebar
          update={update}
          tabsHidden={sidebarEnabled()}
          hidden={titlebarHidden()}
          debugTools={debugTools && { visible: debugTools.visible(), toggle: debugTools.toggle }}
        />
        <div class="flex flex-1 min-h-0 min-w-0 flex-row">
          <Show when={sidebarEnabled()}>
            <aside
              ...
              style={{ width: `${layout.navSidebar.width()}px` }}
            >
              <Suspense>
                <NavSidebar
              ...
              <ResizeHandle
                ...
                size={layout.navSidebar.width()}
                min={200}
                max={520}
                onResize={layout.navSidebar.resize}
              />
            </aside>
          </Show>
