...
import { scrollTabIntoView, useTabNavigationCommands } from "@/shell/tabs/tab-commands"
import { useTabCapacity } from "@/shell/tabs/use-capacity"
...
export function Titlebar(props: {
  ...
  tabsHidden?: boolean
  /**
   * Web build with the nav sidebar: nothing is left for the bar to show, so the
   * `<header>` is not rendered at all. The tab bookkeeping below (route → tab,
   * capacity, mod+N/mod+B/mod+W commands) still runs — it lives here because
   * this is the one component mounted in every layout.
   */
  hidden?: boolean
}) {
  ...
  const updateState = createMemo<TitlebarUpdatePillState>(() => {
    ...
    return {
      // With the nav sidebar on, its Update button replaces the pill.
      visible: (version !== undefined || installing) && !props.tabsHidden,
      installing,
    ...
    <Switch>
      <Match when>
        {(_) => {
          ...
          // Strip mode navigates the visibility-filtered order (a tab whose session has
          // not resolved is not rendered, so mod+N must skip it) and scrolls the target
          // into view. Sidebar mode uses the raw store, now bounded by useTabCapacity.
          const [visibleOrder, setVisibleOrder] = createSignal<readonly Tab[] | undefined>()
          const navigationTabs = () => (props.tabsHidden ? tabsStore : (visibleOrder() ?? tabsStore))

          useTabNavigationCommands({
            tabs: navigationTabs,
            currentTab,
            onNavigate: (tab) => {
              tabs.select(tab)
              scrollTabIntoView(tabKey(tab))
            },
          })

          useTabCapacity({ currentTab })
            ...
            <Show when={!props.hidden}>
              ...
              >
                ...
                >
                  ...
                  <Show when={!props.tabsHidden}>
                    <TitlebarTabStrip
                      tabs={tabsStore}
                      currentTab={currentTab()}
                      forceTruncate={tabsAreOverflowing()}
                      onOverflowChange={setTabsAreOverflowing}
                      onVisibleTabsChange={setVisibleOrder}
                      onNavigate={(tab, el) => {
                        tabs.select(tab)
...
export function ChannelIndicator(props: { debugTools?: { visible: boolean; toggle: () => void } }) {
