...
describe("updaterAction", () => {
  ...
  test("projects updater transitions into one settings action", () => {
    ...
    expect(updaterAction({ status: "restarting", version: "2.0.0" })).toEqual({
      label: "settings.updates.action.restarting",
    })
    expect(updaterAction({ status: "manual", version: "2.0.0", command: "npm i -g force-agent@2.0.0" })).toEqual({
      label: "toast.update.action.installRestart",
      run: "manual",
    })
  })
