      ...
      >
        ...
        >
          {props.review.hasChanges()
            ? language.plural("session.review.filesChanged", props.review.count())
...
export function SessionDesktopReview(props: { review: SessionReviewModel; present?: boolean }) {
  return (
    <Suspense>
      <ReviewPanelContent review={props.review} />
    </Suspense>
  )
}
...
export function ReviewPanelContent(props: { review: SessionReviewModel }) {
  return (
...
export function ReviewTitle(props: { review: SessionReviewModel }) {
  const language = useLanguage()
