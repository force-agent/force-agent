...
export default defineConfig({
  ...
  projects: [
    ...
    // Opt-in, because it needs a real Chrome on the machine: PLAYWRIGHT_CHROME=1.
    // Bundled Chromium and shipping Chrome do not always lay out the same, and the
    // gap is not academic — Chrome 151 refused to treat a flex-resolved height as a
    // definite base for a percentage child, so the session panel grew to its content
    // and pushed the composer thousands of pixels below the fold. Every bundled run
    // passed while the product was unusable on a MacBook. Run this before shipping a
    // layout change.
